...
for inputBit in range(numBits):    
    ...
    if not testBit(inputBit, opsMap):
        ...
        fixed = False
        while (candidate := next(candidates, None)) != None:
            a, b = candidate
            swap(a, b, opsMap)
        ...
        if not fixed:
            candidatePairs = combinations(candidateSwaps(inputBit, list(opsMap.items())), 2)
            while (candidatePair := next(candidatePairs, None)) != None:
                (a, b),(c,d) = candidatePair
